...
2. **Top failure modes.** The four most likely _technical_ failure modes for this project, ranked by severity. (Not commercial risks — engineering risks: e.g., calendar drag-and-drop on RTL mobile, template approval delays, audit log performance, etc.)
