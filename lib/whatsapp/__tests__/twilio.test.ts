...
vi.mock('@/lib/db', () => {
  ...
  let missingSids: Array<{ name: string; language: LanguagePref }> = [];
  return {
    db: {
      whatsAppTemplate: {
        findUnique: vi.fn(async () => template),
        // Prompt 45: the extended health check sweeps for active templates
        // with no twilioContentSid.
        findMany: vi.fn(async () => missingSids),
      },
    },
    __setTemplate: (t: typeof template) => {
      template = t;
    },
    __setMissingSids: (rows: Array<{ name: string; language: LanguagePref }>) => {
      missingSids = rows;
    },
  };
});
...
const setMissingSids = (
  dbModule as unknown as {
    __setMissingSids: (rows: Array<{ name: string; language: string }>) => void;
  }
).__setMissingSids;
...
describe('TwilioWhatsAppProvider.healthCheck', () => {
  ...
  it('returns true when account.fetch resolves AND every active template has a Content SID', async () => {
    const provider = new TwilioWhatsAppProvider({ client: fakeClient() });
    await expect(provider.healthCheck()).resolves.toBe(true);
  });

  it('goes LOUDLY red when an active template is missing its Content SID, naming it (Prompt 45)', async () => {
    setMissingSids([{ name: 'appointment_reminder_v2', language: 'AR' }]);
    const warn = vi.spyOn(console, 'warn').mockImplementation(() => {});
    const provider = new TwilioWhatsAppProvider({ client: fakeClient() });
    await expect(provider.healthCheck()).resolves.toBe(false);
    expect(warn.mock.calls.flat().join(' ')).toContain('appointment_reminder_v2/AR');
    warn.mockRestore();
  });

  it('fails with a clear message when credentials are missing', async () => {
    const provider = new TwilioWhatsAppProvider({
      client: fakeClient(),
      accountSid: '',
      authToken: '',
