...
export class TwilioWhatsAppProvider implements WhatsAppProvider {
  ...
  /**
   * Extended health check (Prompt 45 §2.4): credentials AND the Content SID
   * mapping must both be sound. A template the code can send that has no
  ...
  async healthCheck(): Promise<boolean> {
    if (!this.accountSid || !this.authToken || !this.from) {
      console.warn(
        '[whatsapp.twilio] health check failed: missing credentials — set ' +
    ...
    try {
      ...
      ok = !!account?.sid;
      if (!ok) console.warn('[whatsapp.twilio] health check failed: account fetch returned empty');
    } catch (err) {
      console.warn(`[whatsapp.twilio] health check failed: ${describeWhatsAppError(err)}`);
      ok = false;
    }
    try {
      const missing = await db.whatsAppTemplate.findMany({
        where: { active: true, twilioContentSid: null },
        select: { name: true, language: true },
        orderBy: [{ name: 'asc' }, { language: 'asc' }],
      });
      if (missing.length > 0) {
        console.warn(
          '[whatsapp.twilio] health check: MISSING Content SIDs for active template(s): ' +
            missing.map((t) => `${t.name}/${t.language}`).join(', ') +
            ' — paste the approved HX… SIDs per docs/whatsapp-twilio.md §3.',
        );
        ok = false;
      }
    } catch (err) {
      console.warn(
        `[whatsapp.twilio] health check: SID mapping query failed: ${describeWhatsAppError(err)}`,
      );
      ok = false;
    }
    return ok;
