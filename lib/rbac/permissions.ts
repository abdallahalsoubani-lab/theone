...
export const PERMISSIONS = {
  ...
  // ── Pediatric assessment (Prompt 21) — clinical content ───────────────
  // DOCTOR + ADMIN create/edit and manage the custom fields; THERAPIST reads
  // (assigned patients) only; SECRETARY has NO access (clinical content);
  // patient portal not shown.
  PEDIATRIC_ASSESSMENT_CREATE: 'pediatric_assessment.create',
  PEDIATRIC_ASSESSMENT_READ_ASSIGNED: 'pediatric_assessment.read.assigned',
  PEDIATRIC_ASSESSMENT_UPDATE: 'pediatric_assessment.update',
  PEDIATRIC_ASSESSMENT_MANAGE_FIELDS: 'pediatric_assessment.manage_fields',

  // ── Patient documents (Prompt 22) — external uploads on the patient file ─
  // SECRETARY + ADMIN: full (unscoped). DOCTOR: upload/delete + read assigned.
  // THERAPIST: read assigned only. Visibility follows the patient file.
  PATIENT_DOCUMENTS_READ: 'patient_documents.read',
  PATIENT_DOCUMENTS_READ_ASSIGNED: 'patient_documents.read.assigned',
...
const DOCTOR_PERMS = new Set<PermissionCode>([
  ...
  PERMISSIONS.PEDIATRIC_ASSESSMENT_MANAGE_FIELDS,
  PERMISSIONS.PATIENT_DOCUMENTS_READ_ASSIGNED,
  PERMISSIONS.PATIENT_DOCUMENTS_UPLOAD,
  PERMISSIONS.PATIENT_DOCUMENTS_DELETE,
  PERMISSIONS.NOTIFICATIONS_READ_OWN,
  PERMISSIONS.NOTIFICATIONS_MARK_READ_OWN,
...
const THERAPIST_PERMS = new Set<PermissionCode>([
  ...
  PERMISSIONS.PATIENTS_READ_ASSIGNED,
  PERMISSIONS.INTAKE_READ_ASSIGNED,
  // Therapist reads pediatric assessments (assigned patients) — no write,
  // no field management (Prompt 21 §6).
  PERMISSIONS.PEDIATRIC_ASSESSMENT_READ_ASSIGNED,
  // Patient documents: read/download assigned patients only — no upload/delete.
  PERMISSIONS.PATIENT_DOCUMENTS_READ_ASSIGNED,
  PERMISSIONS.NOTIFICATIONS_READ_OWN,
  PERMISSIONS.NOTIFICATIONS_MARK_READ_OWN,
]);
