  ...
  return (
    <section className="mx-auto max-w-4xl space-y-6 p-6">
      <header className="space-y-2">
        {/* Back to the patient file — parity with the doctor plan page (6B). */}
        <Link
          href={`/therapist/patients/${plan.patientId}` as `/${string}`}
      ...
      <PlanCard
        plan={plan}
        viewerRole={(session?.user?.role ?? 'THERAPIST') as 'THERAPIST'}
        editHref={plan.status === 'ACTIVE' ? `/therapist/plans/${plan.id}/edit` : undefined}
      />
    </section>
  );
}
